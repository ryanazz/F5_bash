...
Server=tcfabric-pncp-02.mla1.dev.sc.local
Port=443
...
cmd=(dialog --separate-output --checklist "TCFABRIC-PNCPP-02.MLA1.DEV.SC.LOCAL" 18 76 16)
...
do
    case $choice in
        1)
	    echo -e "${GREEN}Enabling tcfabric-pncp-02.mla1.dev.sc.local in wap-pncpadmin-pool-https Pool${NONE}"
	    ...
	    echo -e "\n${GREEN}tcfabric-pncp-02.mla1.dev.sc.local enabled on wap-pncpadmin-pool-https Pool${NONE}"
            ;;
        2)
	    echo -e "${GREEN}Enabling tcfabric-pncp-02.mla1.dev.sc.local in wap-pncpadmin-console-pool-https Pool${NONE}"
	    ...
	    echo -e "\n${GREEN}tcfabric-pncp-02.mla1.dev.sc.local enabled on wap-pncpadmin-console-pool-https Pool${NONE}"
            ;;
        3)
	     echo -e "${GREEN}Enabling tcfabric-pncp-02.mla1.dev.sc.local in wap-pncpadmin-shell-pool-https Pool${NONE}"
	     curl -sk -u $LTM_CREDENTIALS https://"$LTM_IP"/"$BASE_LTM_URL"/wap-pncpadmin-shell-pool-https/members/~Common~"$Server":"$Port"/ -H "Content-Type: application/json" -X PUT -d '{"session": "user-enabled"}'
            echo -e "\n${GREEN}tcfabric-pncp-02.mla1.dev.sc.local enabled on wap-pncpadmin-shell-pool-https Pool${NONE}"
            ;;
	4)
	     echo -e "${RED}Disabling tcfabric-pncp-02.mla1.dev.sc.local from wap-pncpadmin-pool-https Pool${NONE}"
	     ...
	     echo -e "\n${RED}tcfabric-pncp-02.mla1.dev.sc.local disabled from wap-pncpadmin-pool-https Pool${NONE}"
	    ;;
	5)
             echo -e "${RED}Disabling tcfabric-pncp-02.mla1.dev.sc.local from wap-pncpadmin-console-pool-https Pool${NONE}"
	     curl -sk -u $LTM_CREDENTIALS https://"$LTM_IP"/"$BASE_LTM_URL"/wap-pncpadmin-console-pool-https/members/~Common~"$Server":"$Port"/ -H "Content-Type: application/json" -X PUT -d '{"session": "user-disabled"}'
             echo -e "\n${RED}tcfabric-pncp-02.mla1.dev.sc.local disabled from wap-pncpadmin-console-pool-https Pool${NONE}"
            ;;
	6)
             echo -e "${RED}Disabling tcfabric-pncp-02.mla1.dev.sc.local from wap-pncpadmin-shell-pool-https${NONE}"
             ...
             echo -e "\n${RED}tcfabric-pncp-02.mla1.dev.sc.local disabled from wap-pncpadmin-shell-pool-https${NONE}"
            ;;
